...
import {
  ...
  useExamScheduleQuery,
  useGetAllexamsQuery,
  useSetExamFeeMutation,
...
export default function ExamSettings() {
  ...
  const [setExamFee, { isLoading: isSettingFee }] = useSetExamFeeMutation();
  ...
  const [feeFormData, setFeeFormData] = useState({ fee: '', lateFee: '' });
  const [feeFeedback, setFeeFeedback] = useState<FeedbackState | null>(null);
  ...
  const isBusy = isCreating || isDeleting || isSettingSchedule || isSettingFee;
  ...
  const handleSelectExam = (exam: Exam) => {
    ...
    setFeeFormData({ fee: String(exam.fee ?? ''), lateFee: String(exam.lateFee ?? '') });
    setFeeFeedback(null);
    setFeedback(null);
  };
  ...
  const handleOpenFeeEditor = () => {
    ...
    setFeeFormData({
      fee: String(selectedExamInFilteredList.fee ?? ''),
      lateFee: String(selectedExamInFilteredList.lateFee ?? ''),
    });
    setFeeFeedback(null);
    setIsFeeEditorOpen(true);
  };

  const handleFeeInputChange = (event: ChangeEvent<HTMLInputElement>) => {
    const { name, value } = event.target;
    setFeeFormData((currentData) => ({
      ...currentData,
      [name]: value,
    }));
    if (feeFeedback?.kind === 'error') {
      setFeeFeedback(null);
    }
  };

  const handleCancelFeeEditor = () => {
    if (selectedExamInFilteredList) {
      setFeeFormData({
        fee: String(selectedExamInFilteredList.fee ?? ''),
        lateFee: String(selectedExamInFilteredList.lateFee ?? ''),
      ...
      setFeeFormData({ fee: '', lateFee: '' });
    }
    setFeeFeedback(null);
    setIsFeeEditorOpen(false);
  };

  const handleSaveFee = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!selectedExamInFilteredList) {
      return;
    }
    const fee = Number(feeFormData.fee);
    const lateFeeValue = feeFormData.lateFee.trim();
    ...
    try {
      ...
      setFeeFeedback({
        kind: 'success',
        text: 'Exam fees updated successfully.',
      });
      setIsFeeEditorOpen(false);
      await refetch();
    } catch (mutationError) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-6 sm:px-6 lg:px-8">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-3">
                        ...
                        <div className="rounded-2xl bg-slate-50 p-3">
                          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Late Fee</p>
                          <p className="mt-2 text-sm font-semibold text-slate-900">{exam.lateFee !== undefined ? formatAmount(exam.lateFee) : 'Not set'}</p>
                        </div>
                      </div>
                    </button>
                  );
                ...
                {selectedExamInFilteredList ? (
                  <div className="space-y-5">
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-white p-5">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          <p className="text-sm font-semibold text-emerald-600">Exam fees</p>
                          <h4 className="mt-1 text-lg font-semibold text-slate-900">
                            Change fees for the selected exam
                          </h4>
                          <p className="mt-2 text-sm leading-6 text-slate-500">
                            Update the registration fee and optional late fee for this exam. Changes are saved immediately to the exam record.
                          </p>
                        </div>
                      ...
                      {isFeeEditorOpen ? (
                        <form onSubmit={handleSaveFee} className="mt-5 space-y-5">
                          {feeFeedback ? (
                            <div
                              className={`rounded-2xl px-4 py-3 text-sm font-medium ${
                                feeFeedback.kind === 'success'
                                  ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-200'
                                  : 'bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-200'
                              }`}
                            >
                          ...
                          <div className="grid gap-4 sm:grid-cols-2">
                            <div>
                              <label htmlFor="selectedExamFee" className="mb-2 block text-sm font-medium text-slate-700">
                                Registration fee
                              </label>
                              <input
                                id="selectedExamFee"
                                ...
                                placeholder="0.00"
                                className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500"
                              />
                            </div>

                            <div>
                              <label htmlFor="selectedExamLateFee" className="mb-2 block text-sm font-medium text-slate-700">
                                Late fee (optional)
                              </label>
                              <input
                                id="selectedExamLateFee"
                                name="lateFee"
                                type="number"
                                min="0"
                                step="0.01"
                          ...
                          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            >
                              <FaMoneyBillWave />
                              {isSettingFee ? 'Saving fees...' : 'Save fees'}
                            </button>
                          </div>
                        </form>
                      ) : (
                        <div className="mt-5 rounded-2xl bg-slate-50 px-4 py-4 text-sm text-slate-600 ring-1 ring-inset ring-slate-200">
                          Current fees: {formatAmount(selectedExamInFilteredList.fee)}
                          {selectedExamInFilteredList.lateFee !== undefined && selectedExamInFilteredList.lateFee !== null
                            ? ` · Late fee: ${formatAmount(selectedExamInFilteredList.lateFee)}`
                            : ' · No late fee set'}
                        </div>
                      )}
        ...
        {/* All Exam Schedules Section */}
        <AllExamSchedulesSection />
      </div>
    </div>
  );
}

function AllExamSchedulesSection() {
  const { data: schedulesData, isLoading, error } = useExamScheduleQuery();

  const normalizeSchedules = (raw: unknown): Array<{ examName?: string; startDate?: string; lateDate?: string; endDate?: string; fee?: number; lateFee?: number }> => {
    if (!raw) return [];
    if (Array.isArray(raw)) return raw as Array<{ examName?: string; startDate?: string; lateDate?: string; endDate?: string; fee?: number; lateFee?: number }>;
    const nested = (raw as { data?: unknown; schedules?: unknown; results?: unknown }) || {};
    if (Array.isArray(nested.schedules)) return nested.schedules as Array<{ examName?: string; startDate?: string; lateDate?: string; endDate?: string; fee?: number; lateFee?: number }>;
    if (Array.isArray(nested.data)) return nested.data as Array<{ examName?: string; startDate?: string; lateDate?: string; endDate?: string; fee?: number; lateFee?: number }>;
    if (Array.isArray(nested.results)) return nested.results as Array<{ examName?: string; startDate?: string; lateDate?: string; endDate?: string; fee?: number; lateFee?: number }>;
    // Handle nested data.schedules structure from API
    if (nested.data && typeof nested.data === 'object' && 'schedules' in (nested.data as object)) {
      const d = nested.data as { schedules?: unknown };
      if (Array.isArray(d.schedules)) return d.schedules as Array<{ examName?: string; startDate?: string; lateDate?: string; endDate?: string; fee?: number; lateFee?: number }>;
    }
    return [];
  };

  const schedules = normalizeSchedules(schedulesData);
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-slate-900">All Exam Schedules</h2>
            <p className="mt-1 text-sm text-slate-500">
              Complete history of all exam schedules ever configured by administrators.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        {isLoading ? (
          <div className="py-12 text-center">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-2 border-slate-200 border-t-emerald-600"></div>
            <p className="text-sm text-slate-600">Loading exam schedules...</p>
          </div>
        ) : error ? (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-6 text-center">
            <p className="text-sm text-rose-600">Failed to load exam schedules. Please try again.</p>
          </div>
        ) : schedules.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
            <h3 className="text-lg font-semibold text-slate-900">No schedules found</h3>
            <p className="mt-2 text-sm text-slate-500">
              No exam schedules have been configured yet.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Exam Name</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Start Date</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Late Entry</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">End Date</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Fee</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Late Fee</th>
                </tr>
              </thead>
                ...
                {schedules.map((schedule, index) => (
                  <tr key={index} className="hover:bg-slate-50/50">
                    <td className="px-6 py-4">
                      <p className="font-medium text-slate-900">{schedule.examName || 'Unnamed Exam'}</p>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">{formatDateTime(schedule.startDate)}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{formatDateTime(schedule.lateDate)}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{formatDateTime(schedule.endDate)}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {schedule.fee !== undefined ? `₦${schedule.fee.toLocaleString()}` : 'N/A'}
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {schedule.lateFee !== undefined && schedule.lateFee !== null ? `₦${schedule.lateFee.toLocaleString()}` : 'N/A'}
                    </td>
                  </tr>
                ))}
              </tbody>
